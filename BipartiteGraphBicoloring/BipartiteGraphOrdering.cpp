...
namespace ColPack
{
	...
	int BipartiteGraphOrdering::RandomOrdering()
	{
		...
		pair<int, int>* listOfPairs = new pair<int, int>[i_LeftVertexCount];
		...
		sort(listOfPairs,listOfPairs + i_LeftVertexCount);

		//Now, populate vector m_vi_OrderedVertices with the left vertices
		for(unsigned int i = 0; i<i_LeftVertexCount; i++) {
			m_vi_OrderedVertices.push_back(listOfPairs[i].second);
		}
		...
		/*
		listOfPairs = new pair<int, int>[i_RightVertexCount];

		//populate listOfPairs
		for(unsigned int i = 0; i<i_RightVertexCount; i++) {
			listOfPairs[i].first = rand();
			listOfPairs[i].second = i + i_LeftVertexCount;
		}

		sort(listOfPairs,listOfPairs + i_RightVertexCount);
